...
Uma pequena amostra do terminal dos ambientes Unixes.

# Navegação

## pwd

Exibe o diretório corrente, equivalente as barras que informam o endereço do site atual.
...
``-a`` ou ``-A``: ambos mostram os arquivos ocultos (com o primeiro caractere _._, ponto), a diferença existe em que o minúsculo mostra _.._, diretório superior, e _._, diretório atual, enquanto o maiúsculo não os mostra.
...
* ``-a``: mantêm as permissões do arquivo e preserva links.

Nota: sempre é necessário informar o diretório ou o arquivo de destino, se o diretório de destino for o atual use _._ (ponto) como endereço; Caso queira copiar arquivos de outro computador veja o comando **scp** - existe um artigo somente para conexões: [Conexões Remotas](/conexoes-remotas).
...
* ``-f``: não pede confirmação, ou seja, força a exclusão mesmo se a opção **-i** seja usada;
...
Grande parte dos arquivos de configuração dos programas são armazenados em formato texto. Além deste tópico sugiro que leia, aqui mesmo, [Editor de Arquivos vi](/vim) pois na maioria das falhas dos sistema é melhor configurar para corrigir o erro na linha de comando e um bom editor como **vi** é a minha indicação.
...
Claro que este exemplo pode não ser muito útil, mas use qualquer comando com uma lista; o conteúdo entre **` `** é substituindo como se informado manualmente, neste exemplo é como se o **cat** fosse executado primeiro e somente então fornecido o valor para o **ls**.
...
A saída de um comando pode ser passa para um arquivo, por exemplo: ``ls -l > lista`` gera uma lista dos arquivos do diretório corrente e coloca, "imprime", no arquivo lista.
